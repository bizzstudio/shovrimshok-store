...
const DeliveryPage = ({ params }) => {
  ...
  const { data, loading, error } = useAsync(() => CustomerServices.getDocumentById(deliveryId, "DeliveryNotes"));
  ...
  // קומפוננטת סקלטון לתעודת משלוח (ללא מחירים)
    ...
    <div className="max-w-screen-2xl mx-auto py-10 px-3 sm:px-6">
      <div className="bg-white rounded-md shadow-sm p-8 text-center">
        <div className="flex flex-col items-center justify-center">
          <Image
            src={notFoundImg}
            alt={t("common:deliveryNotFound")}
            width={270}
            height={270}
            className="mb-6 select-none"
          />
          <h2 className="text-2xl font-bold text-gray-800 mb-4">
            {t("common:deliveryNotFound")}
          </h2>
          <p className="text-gray-600 mb-6">
            {error?.response?.data?.message?.[currentLang] || "אירעה שגיאה בטעינת תעודת המשלוח"}
          </p>
  ...
  return (
    <Layout title={t("common:deliveryDetails")} description={t("common:deliveryDetailsDescription")}>
      <div className="max-w-screen-2xl mx-auto py-10 px-3 sm:px-6">
        {loading ? (
          <DeliverySkeleton />
        ) : error ? (
            ...
            <div className="bg-white p-8 rounded-b-xl">
              <div className="flex lg:flex-row-reverse md:flex-row-reverse sm:flex-row-reverse flex-col justify-between invoice-btn">
                <ReactToPrint
                  ...
                  documentTitle={`Delivery-${data?.DocNum}`}
